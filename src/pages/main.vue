			...
			<el-row :gutter="1" class="container">
				...
				<el-col :span="15" class="bg-purple">
					<div class="sessionHead">
						<div class="chatLeft">
							<el-image :src="cur_session.avatar_url ? cur_session.avatar_url : require('../static/img/logo.png')" class="chatAvatar"
							 alt="用户头像"></el-image>
							<span class="friendName">{{cur_session.is_group ? cur_session.group_name : cur_session.friend_notes}}</span>
						</div>
...
<script>
	...
	export default {
		...
		mounted(){
			console.log(this.sessions.session_list[0]);
			this.$store.commit("getCurSession", this.sessions.session_list[0]);
		},
		components: {
			'left-menu': menu,
		...
		methods: {
			sendInfo() {
				this.$store.state.socket_instance.send(JSON.stringify({
					user_id: this.$store.state.user.user_id,
